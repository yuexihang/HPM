...
class Calibrated_Spectral_Mixer(nn.Module):
    ...
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., freq_num=32, spectral_embedding=None):
        super().__init__()
        ...
        # Eigenfunctions (fixed spectral embedding from mesh Laplacian)
    ...
    def forward(self, x):
        ...
        eigen_gate = self.softmax(self.in_project_gates(x_mid) / self.temperature)  # B H N G
...
class Mixer_Block(nn.Module):
    def __init__(
        ...
        self.Attn = Calibrated_Spectral_Mixer(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads,
                                              dropout=dropout, freq_num=freq_num, spectral_embedding=spectral_embedding)
...
class Model(nn.Module):
    ...
    def forward(self, x, fx, T=None):
        ...
        if self.spectral_pos_embedding > 0:
            spectral_pos = self.fixed_spectral_embedding.repeat(x.shape[0], 1, 1)  # B, N, K
