...
def main():
    ...
    # Compute LBO eigenvectors via robust Laplacian
    k = args.freq_num + 5
    mesh = sio.loadmat(args.lbo_path)
    points = mesh['Points']
    points = points / 100  # normalization
    faces = mesh['Faces'][:, 1:]
    faces = faces.astype(int)
    evecs = robust_spectral.meshes_spectral_2d(points, faces, k=k)
    LBO_MATRIX = evecs.numpy()
